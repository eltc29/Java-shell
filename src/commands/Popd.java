package commands;

import fileSystem.*;

/**
 * This class represents the 'popd' command.
 * 
 * @author Victor Ko
 */
public class Popd extends Command {

  /**
   * This constructor sets the necessary instance variables.
   * 
   * @param line The input line from the shell
   */
  public Popd(String line) {
    super(line);
  }

  /**
   * This method checks if the input is a valid line input, i.e there shouldn't be any input
   * arguments.
   * 
   * @param line A String of input arguments
   * @return boolean based on whether above description is satisfied
   */
  @Override
  public boolean validateLine(String line) {
    if (line.equals("")) {
      return true;
    }
    out.toStdErr("Error in popd: Popd does not take in arguments\n");
    return false;
  }

  /**
   * This method returns true since no arguments to check.
   * 
   * @param arg The input argument parsed
   * @return boolean based on whether the above description is satisfied
   */
  @Override
  protected boolean validateArg(String arg) {
    return true;
  }

  /**
   * This method executes the command.
   * 
   * @param arg The argument for which the command is executing
   */
  @Override
  public void execute(String arg) {
    // get directory from the filesystem
    Directory directory = FileSystem.getFileSystem().popd();
    if (directory == null) {
      out.toStdErr("Error in popd: Directory stack is empty\n");
    }
    // sets cwd to the one from filesystem
    FileSystem.getFileSystem().setCWD(directory);
  }

  /**
   * This method just returns an array of empty String since there were no input arguments.
   * 
   * @param args A String of all input arguments
   * @return ret A String array of empty string argument
   */
  @Override
  protected String[] setArgs(String args) {
    String[] ret = {""};
    return ret;
  }

}
